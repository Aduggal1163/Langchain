...
#loops (cycles): Instead of executing nodes once and ending, the graph can repeatedly improve content until a condition is met.
...
def conditional_loop():
    def evaluate_quality(state: QualityState)->dict:
        response = llm.invoke(
            [
                SystemMessage(content='Rate the content quality out of 10. Reply with just the number'),
    ...
    def improve_content(state: QualityState)->dict:
        response = llm.invoke(
            f"Improve the content to be more engaging and clear \n {state['content']}"
        )
        return {
    ...
    def finalize_content(state: QualityState)->dict:
        return {
            ...
            'feedback':f"Approved after {state['iteration']} iterations with the quality score of {state['quality_score']}"
        }
    def shouldcontinue(state:QualityState)->Literal['improve','finalize']:
...
def multi_path_routing():
    ...
    def analyze_task(state: TaskState)->dict:
        urgent_response = llm.invoke(
            f"Is this task urgent? Reply 'urgent' or 'normal'.\nTask: {state['task']}"
        )
        #OR
        #urgent_response = llm.invoke([
        # SystemMessage(content="Is this task urgent? Reply 'urgent' or 'normal'.")
        ...
        return {
            "urgency": urgent_response.content.lower().strip(),
