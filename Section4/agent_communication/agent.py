...
#  Agents communicate through shared message list
#=============================


class MessagePassing(TypedDict):
    messages: Annotated[list[BaseMessage],add_messages]
    current_phase: str

def create_message_passing_pipeline():
    """Agents communicate by appending messages that others can read."""
    def researcher(state: MessagePassing)->dict:
        """Researches the topic and posts findings as a message."""
        response = llm.invoke(
            [
                SystemMessage(content="You are a researcher. Read the user's question."
    ...
    def fact_checker(state:MessagePassing)->dict:
        """Reads the researcher's message and validates the claims."""
        response = llm.invoke(
                    [
                        SystemMessage(content="You are a fact checker. Read the researcher's findings."
                        "in a conversation and validate or challenge them. Keep it to 2-3 sentences.")
                    ] + state["messages"]
                )
        return {
            'messages':[
    ...
    def summarizer(state:MessagePassing)->dict:
        """Reads all the previous messages and creates a final summary."""
        response = llm.invoke(
            [
                SystemMessage(content="You are a summarizer. Read researcher's findings and fact-checker reviews produce a final accurate summary. Keep it to 2-3 sentences")
            ]+ state['messages']
        )
    ...
    graph = StateGraph(MessagePassing)
...
def execution():
    """Execution of the agent message pipelining"""
    app = create_message_passing_pipeline()
    print("Execution of the agent message pipelining")
    result = app.invoke({
        'messages':[HumanMessage(content='What are main benefits of using macOS or windows')],
